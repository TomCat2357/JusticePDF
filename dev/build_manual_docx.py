...
BUTTON_TABLE = [
    ...
    ("結合・分解 ▼", "—",
     "メニューから「結合」（選択した複数のファイル・フォルダを 1 つの PDF に結合、"
     "フォルダ構成はしおりの階層として再現）または「分解」（選択した 1 つの PDF を、"
     "しおりの一番上の階層ごとに複数のファイルへ分解。しおりが無い場合は 1 ページずつ分解）"
     "を選べます。"),
    ("インポート ▼", "—",
    ...
    ("並び替え ▼", "—",
     "メニューから名前順／日付順（各 昇順・降順）または「手動順」（ドラッグで並べた順）に切り替えます。"
     "フォルダカードも対象です。"),
    ("設定", "—",
     "デフォルトで開くフォルダ（既定は「ドキュメント／PDFs」）を変更します。"
     "変更内容は次回起動時から反映されます。"),
]
...
def build_doc() -> Document:
    ...
    _para(
        doc,
        "起動すると、既定では「ドキュメント／PDFs」というフォルダが"
        "「作業フォルダ」（＝今あつかっているフォルダ）として開きます。"
        "起動直後は、他のウィンドウより手前に表示されます。",
    )
    _para(
        doc,
        "この既定の作業フォルダは、ツールバーの「設定」ボタンから変更できます。"
        "「設定」ボタンを押し、「デフォルトで開くフォルダ」欄に新しいフォルダを"
        "入力するか「参照…」ボタンで選び、「OK」を押すと保存されます。"
        "変更した内容は次回アプリを起動したときから反映され、"
        "すでに開いているウィンドウの作業フォルダは切り替わりません。",
    )
    _img(doc, "08_settings_dialog.png", width_cm=10)
    _tip(
        doc,
    ...
    _tip(
        doc,
        "「.zip」ファイルをドラッグ＆ドロップ、または「ファイルをインポート」で選ぶと、"
        "自動で中身を展開し、入っている PDF・Office 文書・画像をまとめて取り込みます。"
        "圧縮解除の際にパスワードが必要な ZIP は取り込めません。",
    )
    ...
    _tip(
        doc,
        "カードの並ぶ列数や大きさは、ウィンドウの幅に合わせて自動で調整されます。"
        "ウィンドウを広げると 1 行に並ぶカードの数が増え、狭めると減ります。",
    )
    ...
    _para(
        doc,
        "ツールバーの「結合・分解」ボタンを押すと、選んでいる内容に応じてメニューに"
        "「結合」または「分解」が表示されます"
        "（フォルダを 1 つ以上、またはファイルを 2 つ以上選んでいるときは「結合」、"
        "PDF を 1 つだけ選んでいるときは「分解」）。",
    )
    _para(
        doc,
        "メニューの「結合」を選ぶと、選択した項目をまとめて 1 つの PDF に結合できます。"
        "フォルダを含めて結合すると、フォルダ構成がしおり（目次）の階層として再現されます。"
        "結合後、元のファイル・フォルダはゴミ箱へ移動します（「元に戻す」で復元できます）。",
    )
    _img(doc, "30_merge_menu.png")
    _para(
        doc,
        "メニューの「分解」は「結合」の逆で、1 つの PDF を複数のファイルへ展開します。"
        "PDF を 1 つだけ選んだ状態でこれを選ぶと、しおりの一番上の階層ごとに区切り、"
        "そのしおりの名前をファイル名にして書き出します。"
        "しおりが 1 つも無い PDF の場合は、1 ページずつ「ファイル名_001p.pdf」のように"
        "ページ番号を付けて分解します（ページ数が 1000 以上のときは 4 桁になります）。",
    )
    _img(doc, "31_split_menu.png")
    _bullet(doc, "分解できるのは、PDF を「1 つだけ」選んでいるときです（フォルダは選べません）。")
    _bullet(
        doc,
        "一番上のしおりより前にページがある場合は、その部分を"
        "「ファイル名_しおり前ページ.pdf」として先頭に書き出します。",
    )
    _bullet(doc, "一番上のしおりにぶら下がっていた子のしおりは、分解後のファイルにそのまま残ります。")
    _bullet(doc, "分解後、元のファイルはゴミ箱へ移動します（「元に戻す」で復元できます）。")
    _para(
        doc,
        "ボタンを押すと「何個のファイルに分解するか」「しおりで分けるか、ページで分けるか」を"
        "確認する画面が出ますので、内容を確かめてから「はい」を押してください。",
    )
    _tip(
        doc,
        "ドラッグ＆ドロップで並べた順番（手動順）は、フォルダごとに自動で記憶されます。"
        "アプリを閉じて次回起動しても、並べた順番のまま復元されるので、並べ直す手間はありません。"
        "ツールバーの「並び替え」メニューには「手動順」の項目があり、名前順・日付順に一度切り替えたあとでも、"
        "これを選べばドラッグで並べた順番にいつでも戻せます（並び替えのモード自体も記憶されます）。"
        "アプリの外（エクスプローラーなど）で追加されたファイルは一覧の末尾に、"
        "削除されたファイルは一覧から自然に消えます。",
    )
    ...
    _bullet(doc, "ページ間にドロップ：同一 PDF 内で並べ替え（Ctrl を押しながらだと、並べ替えではなく複製を挿入）")
    ...
    _para(doc, "拡大ビュー上部の操作バーには、次のボタンが並んでいます。")
    _bullet(doc, "「戻る」ボタン：拡大ビューを抜けて元の一覧に戻る")
    _bullet(doc, "「－」「＋」ボタン：縮小・拡大")
    _bullet(doc, "「100%」ボタン：クリックすると 25%〜400% の倍率プリセットをドロップダウンで選択")
    _bullet(doc, "「◁」「▷」ボタン：前後のページへ移動")
    _para(doc, "キーボードでも同じ操作ができます。")
    ...
    _h3(doc, "ページ表示（見開き表示など）")
    _para(
        doc,
        "本のように複数ページを並べて読みたいときは、「ページ表示」ボタンを押して表示方法を"
        "選びます。「1枚」（通常表示）のほか、「横2枚」（見開き表示）・「縦2枚」・「4枚」"
        "（2×2のグリッド表示）から選べます。",
    )
    _img(doc, "25_spread_view.png")
    _para(
        doc,
        "「1枚」以外はすべて「閲覧専用（読むだけ）」モードです。そのため、文字選択・注釈編集・"
        "リンク操作、元に戻す／やり直しはできません。ホイールでのズーム、マウス中ボタンでのパン"
        "（移動）、右ドラッグでの範囲拡大、矢印キーでのスクロール（Ctrl で大きく移動）はそのまま"
        "使えます。前後への移動は、選んだレイアウト1画面分のページ数（横2枚・縦2枚なら2ページ、"
        "4枚なら4ページ）ずつ進みます。「1枚」を選び直すと、元の 1 ページ表示に戻ります"
    ...
    _para(
        doc,
        "注釈を付けるには、まず拡大ビューにします（3.10 参照）。次に拡大ビュー上部の"
        "操作バーにある「アノテーション」ボタンを押すと、下の図のような"
        "「注釈パネル」が右側に開きます（「1枚」表示以外のページ表示では開けません）。",
    )
    _img(doc, "07_annotation_panel.png")
    _para(
        doc,
        "パネルは、いちばん上の「Acrobat手書きを表示」トグルと、"
        "「追加」「選択中の注釈」「このページの付箋」の3つのまとまりで構成されています。"
        "「追加」の「テキストボックス」ボタンで文字を書き込める四角い箱を追加でき、"
        "図形ボタン（―、△、○、□、[ ]）で線・三角・楕円・四角・かっこを描けます。"
        "「選択中の注釈」では、幅・高さ・線幅・透明度・文字サイズ・文字色・背景色・線色を、細かく指定できます。",
    )
    _para(
        doc,
        "「Acrobat手書きを表示」は、Acrobat など他のアプリで書き込まれた手書き（Ink）注釈"
        "の表示・非表示を切り替えるトグルです（JusticePDF 自身はこの種類の注釈を作成・編集"
        "できないため、表示するかどうかだけを選べます）。既定は表示（ON）で、タブの切り替えや"
        "複数ページ表示への切り替えでは状態が保たれますが、アプリを再起動すると既定の表示に"
        "戻ります。",
    )
    ...
    _para(
        doc,
        "作ったマーカーは、その上をドラッグせずにクリックすると選べ、"
        "「マークアップの色」ボタンで色を変えたり、別の種類に切り替えたり、"
        "Delete キーで消したりできます（すべて「元に戻す」でやり直せます）。"
        "マーカーの上からドラッグした場合はマーカーではなく、下の文字がドラッグ選択"
        "されます（マーカー越しでも文字を選び直せます）。",
    )
    _para(
        doc,
        "「消しゴム」ボタンを押すと、選択した文字の範囲だけ既存の文字装飾"
        "（マーカー・下線・取り消し線）を消せます。装飾の一部だけを選ぶと、"
        "残りの部分はそのまま残ります。文字を選択せずに、装飾済みの注釈を選んだ状態で"
        "「消しゴム」を押すと、その注釈ごと削除します（いずれも「元に戻す」で復元できます）。",
    )
    _para(
        doc,
        "「マーカー」「U」「S」「消しゴム」の右にある「連続」ボタンを ON にすると、"
        "ツールボタンを押した状態のまま、なぞる（ドラッグ）かクリックするだけで"
        "次々と同じ操作を繰り返し適用できます。「連続」が OFF（既定）のときは、"
        "これまで通り一度クリックするたびに 1 回だけ適用されます。"
        "「連続」を ON にした瞬間や、連続モード中にツールボタンを押した瞬間に"
        "文字を選択中でも、その場では適用されません（選択は解除され、"
        "ツールが構えられた状態になるだけです）。適用されるのは、ツールを構えた"
        "あとにあらためてなぞる（ドラッグ）かクリックしたときです。",
    )
    _para(
        doc,
        "連続モード中にツールを終えるには、押したボタンをもう一度押すか、Esc キーを押すか、"
        "別の作成モードに切り替えます（このときも「連続」ボタン自体は ON のままです）。"
        "「連続」ボタンをもう一度押して OFF にすると、通常モードに戻ります。",
    )
