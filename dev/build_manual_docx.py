...
def build_doc() -> Document:
    ...
    _para(
        doc,
        "文字を選択せずに「マーカー」「U」「S」のいずれかを押すと、ボタンが押された状態のまま"
        "「連続モード」になります。連続モード中は、なぞる（ドラッグ）かクリックするだけで"
        "次々と同じ種類のマーカーを付けられます。連続モードを終えるには、押したボタンを"
        "もう一度押すか、Esc キーを押すか、別の作成モードに切り替えます。",
    )
    _para(
        doc,
        "「消しゴム」ボタンも同じ連続モードで動作し、なぞった／クリックした文字の範囲だけ"
        "既存の文字装飾（マーカー・下線・取り消し線）を消せます。装飾の一部だけを選ぶと、"
        "残りの部分はそのまま残ります（1 回の操作は「元に戻す」で一括して復元できます）。",
    )
